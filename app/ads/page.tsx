...
export default async function AdsPage({ searchParams }: Props) {
  ...
  const [{ data: resumoRaw }, { data: diarioRaw }] = await Promise.all([
    supabase.rpc("resumo_ads", { p_loja_ids: lojas, p_dias: 30 }),
    qDiario,
  ]);
  ...
  return (
    <div className="p-8 text-white">
      ...
      <p className="mt-2 text-slate-400">
        Gasto, retorno e tendência dos anúncios (nível loja, direto da API da Shopee). Use o seletor de <b>loja</b> no topo.
        A análise por anúncio fica no <b>Raio-X</b>.
      </p>
      ...
      {porDia.length > 0 && (
        <div className="mt-8 rounded-2xl border border-slate-800 bg-slate-900 p-6">
          <div className="mb-1 flex flex-wrap items-baseline justify-between gap-3">
            <h2 className="text-xl font-bold">Gasto por dia (30 dias)</h2>
            {diaPico && (
