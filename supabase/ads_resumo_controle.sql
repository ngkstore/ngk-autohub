...
-- v2 (16/set/2026): as recomendações usam o ÚLTIMO dia disponível (<= hoje) em vez
-- de exigir o dia de hoje — se o job ads-recomendacoes falhar, a página mostra o
-- dia anterior com aviso em vez de zerar gasto em risco/orçamento ideal. 'recs_dia' no JSON.
...
returns json language sql stable as $$
  ...
  dia_rec as (
    select max(dia) d from ads_recomendacoes, hoje
  ...
  rec as (
    ...
    from ads_recomendacoes, dia_rec
    where dia = dia_rec.d and (p_loja_ids is null or loja_id = any(p_loja_ids))
    group by 1
  ),
  saldo as (
  ...
  select json_build_object(
    'recs_dia', (select d from dia_rec),
